...
// Arrays to store the readings
float currentReadings[windowSize] = {0};
float voltageReadings[windowSize] = {0};

// Index to keep track of the current position in the window
int currentIndex = 0;
int voltageIndex = 0;
...
// Function to add a new reading and calculate the moving average
float calculateMovingAverage(float newReading, float readings[], int &index) {
...
float Sensors::GetCurrent() { 
    ...
    // Calculate tolerancde of +-2 points. if within tolerance use previous reading
        ...
        retval = calculateMovingAverage(retval, currentReadings, currentIndex);
...
float Sensors::GetVoltage(int pin) { 
    ...
    // Smooth the voltage reading
    retval = calculateMovingAverage(retval, voltageReadings, voltageIndex);
